...
function drawFishers(t) {
  ...
  for (const f of L.fishers) {
    ...
    ctx.beginPath(); ctx.arc(-1.9, 0, 0.5, 0, TAU); ctx.fill();
    ctx.fillStyle = "rgba(238,214,138,0.97)";   // chapeau de paille
    ctx.strokeStyle = rgba(P.line, 0.55); ctx.lineWidth = 0.08;
    ctx.beginPath(); ctx.ellipse(-1.9, -0.02, 0.78, 0.68, 0, 0, TAU); ctx.fill(); ctx.stroke();
    ctx.fillStyle = "rgba(196,168,104,0.9)";   // calotte du chapeau
    ctx.beginPath(); ctx.ellipse(-1.9, -0.02, 0.42, 0.36, 0, 0, TAU); ctx.fill();
    ...
    ctx.beginPath(); ctx.moveTo(-1.7, 0.6); ctx.lineTo(-1.7, 2.4); ctx.stroke();
    ctx.strokeStyle = "rgba(236,232,210,0.9)"; ctx.lineWidth = 0.04;
    ctx.beginPath(); ctx.moveTo(-1.7, 2.4); ctx.lineTo(-1.6, 3.2); ctx.stroke();
    ctx.lineCap = "butt";
    ctx.restore();
  }
}
